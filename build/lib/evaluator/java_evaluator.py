...
class JavaGenEval:
    ...
    def get_lines(self):
        ...
        self.source = "\n".join(self.code)
        # calculate percentage of lines of comments
        ...
        try:
            ...
            print("Java file Supplied has invalid syntax. System only evaluates valid Java source")
            sys.exit(1)
        # general style info details
        ...
        self.details = {"total_lines": str((i + 1)), "lines_of_code": str(lines_of_code),
                        "blank_lines": str(blank_lines), "lines_of_comments": str(lines_of_comments),
                        "longest_line": str(longest_line), "length_long_line": str(length_long_line),
                        "percent_comments": percent_comments, "number_of_classes": str(len(self.classes)-1),
                        "number_of_methods": str(len(self.methods)),
                        "number_of_variables": self.variables.get("number"),
                        "number_of_single": str(len(self.variables.get("single"))),
                        "number_of_other": str(len(self.variables.get("other"))),
                        "number_of_constants": str(len(self.variables.get("constants"))),
                        "number_of_for": str(self.loops.get("For",0)),
                        "number_of_while": str(self.loops.get("While",0)),
                        "number_of_dowhile": str(self.loops.get("DoWhile",0)),
                        "number_of_if": str(self.conds.get("if",0)),
                        "number_of_switch": str(self.conds.get("Switch",0))}

        return self.details

    # method to get names of classes and line info details
    ...
    # method to get names of methods and line info details
    ...
    # method to get types of loops and line info details
    def get_loops(self):
    ...
    # method to get types of conditional details and line info details
    def get_conds(self):
    ...
    # method to get names of variables and constants and line info details
    def get_variables(self):
        # create dictionary for different variables and list for constants
        variables = {}
        constants = []
        tokens = list(javalang.tokenizer.tokenize(self.source))
        x = 0
        while x < len(tokens):
            # get basic type variables, not methods, and not assignments
    ...
    def get_style_eval(self):
        ...
        #  regex for variables and methods
        ...
        for name in self.methods.values():
            if not var_pattern.match(name):
                # divide line by two and round up as javalange counts lines in 2s
                self.style.append("line:%s pos:%s Error: method name '%s' should use upper CamelCase"
                                  % ("N/A", "N/A", name))
